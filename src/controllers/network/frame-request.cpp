...
void Frame_req::CheckIntegrityTag<true>() const {
    reassembled_frames_list.remove_if(
        [&](const std::shared_ptr<Message>& message) {
            ...
            return ! (game.GetNetworkSystem().VMACVerifierTCP())(
...
Frame_req::Frame_req(const socket_t fd, size_t length_total,
    ...
    timeout(TIMEOUT),
#if defined(_MSC_VER)
    expiration_time(game.GetOS().GetTime() + timeout),
#else
...
void Frame_req::UpdateTimestamp() {
    this->expiration_time = game.GetOS().GetTime() + timeout;
}
bool Frame_req::HasExpired() const {
    return (game.GetOS().GetTime() > expiration_time);
}
#else
void Frame_req::UpdateTimestamp() {
