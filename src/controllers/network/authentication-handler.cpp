...
void Authentication::CreateSecureKey(const trillek_list<std::shared_ptr<Message>>& req_list) {
    ...
    for(auto& req : req_list) {
        // We received reply
        req->RemoveVMACTag();
        auto msg = reinterpret_cast<uint8_t*>(req->FrameHeader());
        NetworkController& client = game.GetNetworkSystem();
        auto& v = client.VMACVerifierTCP();
        if ((v)(req->Tail<const uint8_t*>(), msg, req->PacketSize(), 0) && client.SetAuthState(AUTH_SHARE_KEY)) {
            auto pkt = req->Content<KeyReplyPacket>();
            auto key = make_unique<std::vector<uint8_t>>(PUBLIC_KEY_SIZE);
...
void PacketHandler::Process<NET_MSG,AUTH_SEND_SALT>() const {
    ...
    for(auto& msg : req_list) {
        ...
        if (client.SetAuthState(AUTH_KEY_EXCHANGE)) {
            ...
            Crypto::VMAC64(frame.Tail<byte*>(), reinterpret_cast<byte*>(frame.FrameHeader()),
                frame.PacketSize(), Authentication::GetSecretKey()->data(), packet->nonce);
