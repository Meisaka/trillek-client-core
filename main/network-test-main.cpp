...
//  systems.push(&trillek::game.GetFakeSystem());
    ...
    trillek::game.GetNetworkSystem().Initialize("localhost", 7778);
    trillek::game.GetNetworkSystem().SetTCPHandler();


    // Try a bad password
    if (trillek::game.GetNetworkSystem().Connect("localhost", 7777, "my_login", "bad password")) {
        std::cout << "Error : could login with wrong password" << std::endl;
    }

    // Try a good password
    if(trillek::game.GetNetworkSystem().Connect("localhost", 7777, "my_login", "secret password")) {
        auto entity_id = trillek::game.GetNetworkSystem().EntityID();
        for(auto i = 0; i < 10; ++i) {
        ...
        auto pkt = trillek::game.GetNetworkSystem().GetPacketHandler().GetQueue<TEST_MSG,TEST_MSG_TCP>().Poll();
        if (pkt.size() == 10) {
        ...
        auto timestamp = trillek::game.Now().time_since_epoch().count();
        for(auto i = 0; i < 10; ++i) {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));
        pkt = trillek::game.GetNetworkSystem().GetPacketHandler().GetQueue<TEST_MSG,TEST_MSG_UDP>().Poll();
        if (pkt.size() == 10) {
        ...
        timestamp = trillek::game.Now().time_since_epoch().count();
        for(auto i = 0; i < 10; ++i) {
        ...
        while(1) {
            ...
            std::cout << "Echo reply received: " << trillek::game.GetNetworkSystem().GetPacketHandler().GetQueue<TEST_MSG,TEST_MSG_UDP>().Poll().size() << std::endl;
        }
    }
